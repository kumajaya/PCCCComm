...
public class PlcMemory : IDisposable
{
    ...
    /// <summary>
    /// Loads program from embedded .bin resource (generated by PCCCImageTool).
    /// Supports version 1 (no PhysicalAddress) and version 2 (adds PhysicalAddress per file).
    /// 
    /// Resource selection is family-specific:
    ///   - ML1400: looks for resource containing "ML1400"
    ///   - PLC-5: looks for resource containing "PLC5"
    ///   - SLC: loads any other .bin resource (excluding ML1400/PLC5-specific files)
    /// 
    /// For PLC-5, the (fileType=0, fileNumber=0) entry is the flat memory image,
    /// which is used to reconstruct all data and program files.
    /// </summary>
    private void LoadEmbeddedProgram()
    {
        // Whitelist of families that are explicitly validated for embedded .bin loading.
        // If a new family is added in the future, it must be added to this list only
        // after ensuring its memory layout and file types are compatible; otherwise,
        // any stray .bin resource with the same extension would be loaded incorrectly.
        if (_family != PCCCEmulator.EmulationFamily.SlcMicroLogix &&
            _family != PCCCEmulator.EmulationFamily.Ml1400 &&
            _family != PCCCEmulator.EmulationFamily.Plc5)
        {
        ...
        // Select the appropriate resource based on the emulation family.
        if (_family == PCCCEmulator.EmulationFamily.Ml1400)
        {
            resourceName = assembly.GetManifestResourceNames()
                .FirstOrDefault(n => n.EndsWith(".bin", StringComparison.OrdinalIgnoreCase) &&
                                    n.Contains("ML1400", StringComparison.OrdinalIgnoreCase));
        }
        else if (_family == PCCCEmulator.EmulationFamily.Plc5)
        {
            resourceName = assembly.GetManifestResourceNames()
                .FirstOrDefault(n => n.EndsWith(".bin", StringComparison.OrdinalIgnoreCase) &&
                                    n.Contains("PLC5", StringComparison.OrdinalIgnoreCase));
        }
        else // SLC / MicroLogix fallback
        {
            resourceName = assembly.GetManifestResourceNames()
                .FirstOrDefault(n => n.EndsWith(".bin", StringComparison.OrdinalIgnoreCase) &&
                                    !n.Contains("ML1400", StringComparison.OrdinalIgnoreCase) &&
        ...
        {
            Logger.Always(this, $"No embedded program found for family {_family}. Using default data.");
            return;
        }
        ...
        if (magic != 0xDF1A)
        {
            Logger.Always(this, $"Invalid magic number: 0x{magic:X4}. Skipping.");
            return;
        }
        ...
        {
            Logger.Always(this, $"Unsupported binary version: {version}. Skipping.");
            return;
        }
        ...
        long headerEndPos = br.BaseStream.Position;
        Console.WriteLine($"      Header ends at byte {headerEndPos}, {br.BaseStream.Length - headerEndPos} bytes remaining for {fileCount} files.");

        int dataLoaded = 0, progLoaded = 0;
        byte[]? flatMemoryImage = null;

        for (int i = 0; i < fileCount; i++)
        {
            int fileNumber, fileType, numberOfBytes, physicalAddress = 0, dataLength;
            byte[] fileData;
            ...
            try
            {
                ...
                // Version 2 includes a PhysicalAddress field (4 bytes) before dataLength.
                // Version 1 does not have this field.
                if (version >= 2)
                    physicalAddress = br.ReadInt32();

                dataLength = br.ReadInt32();
                fileData = br.ReadBytes(dataLength);
            }
            catch (EndOfStreamException)
            {
                long pos = br.BaseStream.Position;
                Logger.Always(this, $"Unexpected end of stream at byte {pos} while reading file entry {i}. " +
                                    $"Loaded {dataLoaded} data files and {progLoaded} program files so far.");
                break;
            }

            // ─── PLC-5 flat memory image ─────────────────────────────────────────
            // For PLC-5, the (fileType=0, fileNumber=0) entry contains the entire
            // flat memory image. Store it separately instead of skipping it.
            if (_family == PCCCEmulator.EmulationFamily.Plc5 && fileType == 0 && fileNumber == 0)
            {
                flatMemoryImage = fileData;
                continue;
            }
            ...
            {
                // File already exists — merge binary content.
        ...
        // ─── Process flat memory image for PLC-5 ──────────────────────────────
        if (_family == PCCCEmulator.EmulationFamily.Plc5 && flatMemoryImage != null)
        {
            _rwLock.EnterWriteLock();
            try
            {
                // Replace flat memory with the image from backup.
                // This is safe because we never resize or copy — we just point to the
                // image data directly.
                _flatMemory = flatMemoryImage;
                _flatTotalBytes = flatMemoryImage.Length;

                // Sync flat memory → _files using the pre-built offset map.
                foreach (var (fileNum, flatBase) in _flatOffsetByFileNumber)
                {
                    if (!_flatFileTypeByNumber.TryGetValue(fileNum, out int ft))
                        continue;
                    if (!_files.TryGetValue((ft, fileNum), out var fileData))
                        continue;

                    int copyLen = Math.Min(fileData.Length, _flatTotalBytes - flatBase);
                    if (copyLen > 0)
                        Array.Copy(_flatMemory, flatBase, fileData, 0, copyLen);
                }

                // Refresh the hot cache with the newly loaded data.
                InitializeHotCacheLocked();

                Logger.Always(this, $"Loaded flat memory image ({_flatTotalBytes} bytes) into PLC-5 emulator.");
                _programLoaded = true;
            }
            finally
            {
            ...
            // For PLC-5, we don't call RebuildFlatMemory() because _flatMemory is
            // already the complete image and RebuildFlatMemory would overwrite it
            // with the (incomplete) _files content.
            return;
        }
        ...
        // Rebuild flat memory for non-PLC-5 families (SLC / ML1400) to incorporate
        // the newly loaded (or merged) files.
        RebuildFlatMemory();
    }
