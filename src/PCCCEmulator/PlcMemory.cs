...
public class PlcMemory : IDisposable
{
    ...
    /// <summary>
    /// Loads program from embedded .bin resource (generated by ach_to_df1.py or PCCCImageTool).
    /// Supports version 1 (SLC, no PhysicalAddress) and version 2 (ML1400, adds PhysicalAddress per file).
    /// For ML1400, looks for a resource ending with "ML1400.bin".
    /// For SLC, loads any other .bin resource (excluding ML1400-specific files).
    /// </summary>
    private void LoadEmbeddedProgram()
    {
        // Only SLC and ML1400 families currently support embedded .bin loading.
        if (_family != PCCCEmulator.EmulationFamily.SlcMicroLogix &&
            _family != PCCCEmulator.EmulationFamily.Ml1400)
        {
        ...
        if (_family == PCCCEmulator.EmulationFamily.Ml1400)
        {
            resourceName = assembly.GetManifestResourceNames()
                .FirstOrDefault(n => n.EndsWith("ML1400.bin", StringComparison.OrdinalIgnoreCase));
        }
