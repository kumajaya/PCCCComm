...
public abstract class TCPBaseTransport : ITransport
{
    ...
    private bool CloseInternal(uint? expectedGeneration, bool connectionLost, bool fromReceiveLoop = false)
    {
        ...
        if (!claimed)
        {
            ...
            Debug.WriteLine($"{GetType().Name}: timed out waiting for a concurrent teardown");
            return false;
        }
            ...
            if (rxTask != null && !rxTask.IsCompleted && !fromReceiveLoop)
            {
                try
                {
                    if (!rxTask.Wait(ReceiveLoopDrainTimeoutMs))
                        Debug.WriteLine($"{GetType().Name}: receive loop did not exit within {ReceiveLoopDrainTimeoutMs} ms");
                }
    ...
    private async Task ReceiveLoopAsync(CancellationToken ct, uint generation)
    {
        ...
        // Sized to ushort.MaxValue + 1 so no packet whose length is expressed by a
        // 16-bit field can exceed it — the receive loop below relies on this and does
        // not guard against an oversized read.
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            {
                Debug.WriteLine($"{GetType().Name} receive timed out while reading – closing connection");
                break;
            }
            catch (Exception ex)
            {
                ...
                if (!expected)
                    Debug.WriteLine($"{GetType().Name} receive loop exiting: {ex.GetType().Name}: {ex.Message}");
                break;
            }
        }
